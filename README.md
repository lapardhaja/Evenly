...
6. **Password reset** ([Supabase guide](https://supabase.com/docs/guides/auth/passwords#resetting-a-password)): In **Authentication → URL configuration**, add your site to **Redirect URLs** (e.g. `https://evenly.lapardhaja.com/**`). The app sends `resetPasswordForEmail` with `redirectTo` = `https://yoursite.com/#/update-password`. After the user clicks the email link, **`index.html`** stashes auth params in `sessionStorage` (before the PWA service worker), then **`#/update-password`** loads and calls `updateUser({ password })`. Use **implicit** auth flow (configured in code). If email uses **token_hash** + **type** in the query string, we call `verifyOtp`; if tokens are in the hash, we call `setSession`.
