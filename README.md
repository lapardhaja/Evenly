...
6. **Password reset**: In Supabase → **Authentication** → **URL Configuration**, add your app’s URL to **Redirect URLs** (e.g. `https://evenly.lapardhaja.com/**`). Use redirect **`https://yoursite.com/#/reset-password`** (hash before path). The app parses tokens whether Supabase appends them with **`?`** or **`&`** after the route, uses **implicit** auth flow so the email link works even on a **different device** than where you requested reset, and strips tokens from the URL after applying them.
