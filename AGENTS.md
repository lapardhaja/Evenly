...
- **Optional Supabase** — set `VITE_SUPABASE_URL` + `VITE_SUPABASE_ANON_KEY`; run SQL migration. When set, **sign-in is required** (`RequireAuth`); without env vars, app stays local-only with no login gate. **Forgot password** → `resetPasswordForEmail` → `#/update-password` (`UpdatePasswordPage.jsx`, `supabaseAuthCallback.js`, inline script in `index.html`).
